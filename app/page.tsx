    ...
    <Container className="mx-auto p-4 items-center flex flex-col gap-10 w-full h-screen relative">
      ...
      >
        {questionsAndResponses.map(({ user, ai }, index) => (
          <Container key={index} className="px-3">
            {user && (
              <div className="text-gray-600 mb-2">
                <pre style={{ whiteSpace: "pre-wrap" }}>
                  <code>
                    <h1 className="text-xl text-sky-400 font-bold">YOU</h1>
                    <p className="text-gray-400	 text-lg text-justify">{user}</p>
                  </code>
                </pre>
              </div>
            )}
              ...
              <div className="text-gray-700">
                <pre style={{ whiteSpace: "pre-wrap" }}>
                  <code>
                    ...
                    <p className="text-gray-400 text-lg text-justify ai-message">
                      {ai}
                    </p>
                  </code>
                </pre>
        ...
        {/* Loading Indicator */}
        {isLoading && (
          <div className="mt-4 text-gray-700 flex items-center gap-1">
            <p className="text-white animate-thinker pl-[15px] bold">
              KAALAMAN-AI IS THINKING
            </p>
            <Loader className="mt-[5px]" color="red" size="15px" type="bars" />
          </div>
        )}
      </Container>
